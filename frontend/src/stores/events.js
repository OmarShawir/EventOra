  ...
  actions: {
    ...
    async addEvent(ev) {
      ...
      try {
        ...
        // Replace the optimistic placeholder with the server's real record
        // (real id, server-computed fields, etc.) once the API is live.
        const idx = this.events.findIndex((e) => e.id === optimistic.id);
        if (idx !== -1 && saved) this.events[idx] = saved;
      } catch (err) {
        console.warn("[events store] addEvent API call failed, kept optimistic record:", err.message);
      }
      return optimistic;
    },
    ...
    async updateEvent(id, patch) {
      ...
      try {
        ...
        console.warn("[events store] updateEvent API call failed, kept optimistic update:", err.message);
      }
    },
